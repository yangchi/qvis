<template>
    <div class="home">
        <h1>This is a poorly hacked version of qvis done wrong in www. We need help.</h1>
        <p>To be able to visualize something, you need to load some data. We have several options for that:</p>

        <b-container id="QueryManagerContainer" class="table-striped" fluid>
        <b-row class="queryOptionContainer">
            <b-col cols="1" md="auto"><h3>Option 0</h3></b-col>
            <b-col>
                <h3>Query a connection id</h3>
                <div style="margin: 10px 0px;">
                    <form> 
                        <b-row>
                            <b-col>
                                <b-form-input                  v-model="connectionId" id="cidInput" type="text" placeholder="0a1b2c3d4efaceb00c"></b-form-input>
                            </b-col>
                            <b-col cols="1" md="auto"> 
                                <b-button @click="loadConnectionId()"  :disabled="this.connectionId === ''" variant="primary">Query</b-button>
                            </b-col>
                        </b-row>
                    </form>
                </div>
...
<style scoped>
    ...
    .queryOptionContainer {
        padding: 10px;
        text-align: left;
    }

    .queryOptionContainer:nth-of-type(odd) {
        background-color: rgba(0,0,0,.05);
    }

    /* #QueryManagerContainer :nth-child(even){
        background-color: #dcdcdc;
    }
    #QueryManagerContainer :nth-child(odd){
...
<script lang="ts">
    ...
    @Component({})
    export default class QueryManagerContainer extends Vue {
        ...
        public loadConnectionId(){
            // BIG TODO
            // contentsJson = magicFunctionToQueryConnectionId(this.connectionId);
            //this.store.addGroupFromQlogFile({fileContentsJSON: contentsJSON, filename: this.connectionId});
            // Or
            // this.store.loadQlogDirectlyFromURL( { url : "intern/qviz?query=" + this.connectionId, filename: this.connectionId} );
        }
